/*! evtrack -- UI module */
function getCookies(cookeiString) {
    let res = {}
    let cookies = cookeiString.split('; ');
...
(function(window) {
    ...
    var TrackUI = {
        ...
        record: function(config) {
            ...
            TrackUI.socket = io.connect(TrackUI.settings.postServer);

            TrackUI.socket.on('connect', () => {});

            TrackUI.socket.on('auth', (authData) => {
                let cookies = getCookies(document.cookie);
                if (!cookies.id) {
                    let expireDate = new Date();
                    expireDate.setTime(expireDate.getTime()+(TrackUI.settings.cookieExpiry*24*60*60*1000));
                    document.cookie = `id=${authData.id};path=/;expires=${expireDate.toGMTString()}`;
                    _uid = authData.id;
                } else {
                    _uid = getCookies(document.cookie).id;
                }
        ...
        eventHandler: function(e) {
            ...
            if (register) {
                ...
                if ('isTrusted' in e) {
                    var extraInfo = JSON.stringify({trusted:e.isTrusted})
                }
                else {
                    var extraInfo = '{}'
                }
